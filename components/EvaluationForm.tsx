...
const EvaluationForm: React.FC<EvaluationFormProps> = ({ onSaved, currentUser }) => {
  ...
  // تطبيع النصوص العربية لجعل البحث دقيقاً بغض النظر عن الهمزات والياءات
    ...
    <div className="max-w-2xl mx-auto space-y-2 pb-40 px-2">
      ...
      <div className="space-y-2">
        {cards.map((card, index) => {
          ...
          // البحث في البنود المعتمدة والمقترحة معاً وتطبيع النصوص
          ...
          return (
            <div key={card.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-visible relative">
              <div className="bg-slate-50 px-3 py-1.5 flex justify-between items-center border-b border-gray-100 rounded-t-xl">
                <span className="text-[9px] font-black text-slate-400">حركة #{index + 1}</span>
                {cards.length > 1 && (
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="relative" ref={el => { searchRefs.current[card.id] = el; }}>
                  <div 
                    ...
                    className={`w-full bg-gray-50 border rounded-lg p-2 flex items-center gap-3 cursor-pointer transition-all ${isSearchOpen[card.id] ? 'border-blue-400 bg-white ring-2 ring-blue-50' : 'border-gray-100'}`}
                  >
                    ...
                    {selectedItem ? (
                      <div className="flex-1 text-right overflow-hidden">
                        ...
                        <p className="text-[8px] text-blue-500 font-bold">{selectedItem.code}</p>
                      </div>
                    ) : <p className="text-gray-400 text-[10px] font-medium">اختر البند الفرعي...</p>}
                  </div>

                  {isSearchOpen[card.id] && (
                    <div className="absolute top-full left-0 right-0 z-50 bg-white border border-gray-200 rounded-lg shadow-xl mt-1 overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
                      ...
                      <div className="max-h-[180px] overflow-y-auto custom-scrollbar">
                        {filtered.length > 0 ? filtered.map(item => (
                          <div 
                            ...
                            className="p-2 hover:bg-blue-50 cursor-pointer border-b border-gray-50 flex justify-between items-center group"
                          >
                          ...
                          <div className="p-4 text-center">
                            <p className="text-[9px] text-gray-400 font-bold mb-2">لا توجد نتائج</p>
                            <button 
                              onClick={() => handleQuickSuggest(card.id)}
                              className="bg-blue-600 text-white px-3 py-1.5 rounded-lg font-black text-[9px]"
                            >
                ...
                {/* Counter - Compact */}
                {selectedItem && (
                  <div className="flex items-center justify-between bg-blue-50/20 p-2 rounded-lg border border-blue-100/30">
                    <span className="text-[10px] font-black text-blue-700">الكمية</span>
                    <div className="flex items-center gap-1.5 bg-white p-0.5 rounded-md shadow-sm border border-blue-100/50">
                        <button onClick={() => setCards(cards.map(c => c.id === card.id ? {...c, count: Math.max(1, c.count - 1)} : c))} className="w-6 h-6 flex items-center justify-center text-red-400 hover:bg-red-50 rounded"><i className="fas fa-minus text-[9px]"></i></button>
                        <span className="w-6 text-center font-black text-xs text-slate-800">{card.count}</span>
                        <button onClick={() => setCards(cards.map(c => c.id === card.id ? {...c, count: c.count + 1} : c))} className="w-6 h-6 flex items-center justify-center text-blue-500 hover:bg-blue-50 rounded"><i className="fas fa-plus text-[9px]"></i></button>
                    </div>
                  </div>
                )}
              </div>
      ...
      <div className="fixed bottom-20 lg:bottom-6 left-4 right-4 flex gap-2 z-50">
        <button onClick={() => setCards([...cards, { id: Date.now(), itemId: '', count: 1, subType: '', notes: '', answers: {}, generatedEvals: [] }])} className="flex-1 bg-white text-slate-700 border border-gray-200 py-3 rounded-lg font-black shadow-lg text-[11px] flex items-center justify-center gap-2">
          <i className="fas fa-plus"></i> إضافة
        </button>
        <button onClick={handleSave} disabled={isSaving} className="flex-[2] bg-blue-600 text-white py-3 rounded-lg font-black shadow-xl text-[11px] flex items-center justify-center gap-2">
      ...
      {showSuggestModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
           <div className="bg-white w-full max-w-xs rounded-xl p-5 space-y-4 shadow-2xl">
              <div className="text-center">
                 <h4 className="text-[11px] font-black text-gray-800 uppercase tracking-widest">اقتراح بند جديد</h4>
              </div>
              <div className="space-y-2">
                 <input type="text" value={suggestForm.sub_item} onChange={e => setSuggestForm({...suggestForm, sub_item: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 font-bold text-right text-xs" placeholder="اسم البند" />
                 <input type="text" value={suggestForm.code} onChange={e => setSuggestForm({...suggestForm, code: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2 font-bold text-right text-xs" placeholder="الكود" />
              </div>
              <div className="flex gap-2">
                 <button onClick={handleSaveSuggest} disabled={isSaving} className="flex-1 bg-blue-600 text-white py-2 rounded-lg font-black text-[10px]">تأكيد</button>
                 <button onClick={() => setShowSuggestModal(false)} className="flex-1 bg-gray-100 text-gray-400 py-2 rounded-lg font-black text-[10px]">إلغاء</button>
              </div>
           </div>
        </div>
      )}
